from utils.signal import SignalOps, SignalFeatures
    ...
    def add_labels(self, labels, cutedge:Tuple=(500, 1000)):
        ...
        if labels is not None:
            ...
            if len(labels_array) == len(pseudo_label_slice):
                ...
                print(f"Warning: Length mismatch for pseudo-labels. Expected {len(pseudo_label_slice)}, got {len(labels_array)}. Pseudo-labels will not be applied to the tail.")
            ...
            print("Info: No pseudo-labels provided or loaded. The tail end of the data will remain as initially set (e.g., -1).")
...
class SignalAugmentationPipeline:
    def __init__(self, labeled_df:pd.DataFrame,
                 ...
                 window_length=40, lag=20, noise_level=0.1, diff=True):
