...
class NNClassifier(nn.Module):
    def __init__(
        self, input_dim, output_dim, hidden_dim1=128, hidden_dim2=64, dropout_rate=0.2
    ):
...
def _train_fold_worker(args):
    ...
    optimizer = torch.optim.Adam(
        list(cnn_model.parameters()) + list(classifier_layer.parameters()),
        common_train_params["learning_rate"],
        weight_decay=common_train_params["weight_decay"],
    )

    X_train_fold, y_train_fold = (
        signal_df_fold.iloc[train_idx],
        target_fold.iloc[train_idx],
    )
    ...
    train_loader = DataLoader(
        train_dataset,
        batch_size=common_train_params["batch_size"],
        shuffle=True,
        num_workers=0,
    )  # num_workers=0 for simplicity in multiprocessing
    ...
    for epoch in range(common_train_params["num_epochs"]):
        ...
        epoch_train_accuracy = (
            epoch_train_correct / epoch_train_total if epoch_train_total > 0 else 0
        )
        ...
        avg_epoch_val_loss = (
            epoch_val_loss / len(val_loader) if len(val_loader) > 0 else float("inf")
        )
        epoch_val_accuracy = (
            epoch_val_correct / epoch_val_total if epoch_val_total > 0 else 0
        )
        ...
        if common_train_params.get("verbose_epoch", False):  # Control epoch verbosity
            print(
                f"[Fold {fold_num+1} Epoch {epoch+1}] Train Loss: {avg_epoch_train_loss:.4f}, Val Loss: {avg_epoch_val_loss:.4f}, Val F1 (NN): {epoch_val_f1:.4f}"
            )
        ...
        if avg_epoch_val_loss < best_val_loss_nn:
            ...
            if patience_counter >= common_train_params["early_stopping"]:
                if common_train_params.get("verbose", False):
                    print(
                        f"Fold {fold_num+1} NN early stopping at epoch {epoch+1} on device {device_str}"
                    )
                break
    ...
    if best_fold_nn_model_state:
        ...
        if device_str != "cpu":  # Assumes device_str is 'cuda:X'
            ...
            xgb_gpu_options = {
                "tree_method": "hist"
            }  # 'hist' is generally good for CPU too

        classifier_xgb = XGBClassifier(
            **classifier_params_dict, **xgb_gpu_options, random_state=SEED + fold_num
        )
    ...
    if common_train_params.get("verbose", False):
        val_f1_nn_print = (
            best_fold_nn_model_state.get("val_f1_nn", -1)
            if best_fold_nn_model_state
            else -1
        )
        print(
...
class HybridModel:
    def __init__(
        self, cnn_params, classifier_params, device=None
    ):  # Allow device to be None initially
        ...
        if device is None:  # If no device preference, try CUDA, then CPU
            if torch.cuda.is_available():
                try:
                    ...
                    print(
                        "CUDA is available. Main HybridModel instance will prefer CUDA if not overridden by training."
                    )
                except RuntimeError as e:
                    print(
                        f"CUDA available but got runtime error during test: {e}. Defaulting main instance to CPU."
                    )
            ...
            print(
                "Warning: Requested 'cuda' but torch.cuda.is_available() is False. Main instance falling back to CPU."
            )
            # resolved_device_str remains 'cpu'
            ...
            try:
                ...
                resolved_device_str = "cuda"
                print(
                    "CUDA is available as requested. Main HybridModel instance set to CUDA."
                )
            except RuntimeError as e:
                print(
                    f"Requested 'cuda', but got runtime error: {e}. Main instance falling back to CPU."
                )
                # resolved_device_str remains 'cpu'
        else:  # Explicitly 'cpu' or some other device string
            resolved_device_str = device
            print(
                f"Main HybridModel instance device explicitly set to: {resolved_device_str}"
            )
        ...
        try:
            ...
            if "CUDA-capable device(s) is/are busy or unavailable" in str(
                e
            ) and self.device.startswith("cuda"):
                print(
                    f"CUDA error during CNNCore initialization on {self.device}. Attempting to initialize on CPU instead for the main model instance."
                )
                self.device = "cpu"  # Fallback to CPU for the main instance
                self.cnn_model = CNNCore(**cnn_params).to(self.device)
                print(
                    f"Main HybridModel CNNCore successfully initialized on CPU after CUDA failure."
                )
            else:
    ...
    def train(
        ...
        if model_save_path is None:
            ...
            model_save_path = os.path.join(
                default_save_dir,
                f'hybrid_model_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}',
            )
        ...
        if gpu_ids and torch.cuda.is_available():
            ...
            if not valid_gpu_ids:
                print(
                    "Warning: Specified GPU IDs are invalid or no GPUs available. Falling back to CPU."
                )
                available_devices_str = ["cpu"]
            else:
                available_devices_str = [f"cuda:{gid}" for gid in valid_gpu_ids]
        elif (
            torch.cuda.is_available() and not gpu_ids
        ):  # Use first available GPU if none specified
            available_devices_str = [f"cuda:0"]
            if verbose:
                print(
                    f"No GPU IDs specified, using default: {available_devices_str[0]}"
                )
        else:  # CPU
            available_devices_str = ["cpu"]
            if gpu_ids and verbose:
                print(
                    "Warning: GPUs specified but torch.cuda.is_available() is False. Using CPU."
                )
        ...
        if verbose:
            ...
            print(
                f"Will use up to {num_parallel_processes} parallel processes. Devices available for workers: {available_devices_str}"
            )
        ...
        for i, (train_idx, val_idx) in enumerate(
            skf.split(signal_df_numpy, target_numpy)
        ):  # Use numpy for skf.split
    ...
    def train_process_plot(self, save=True, val_loss_log=False, filter_coeff=None):
            ...
            if filter_coeff:
                mean_loss = np.mean(
                    [
                        l
                        for l in loss_list
                        if l is not None and not np.isinf(l) and not np.isnan(l)
                    ]
                )
                std_loss = np.std(
                    [
                ...
                if not (
                    np.isinf(mean_loss)
                    or np.isnan(mean_loss)
                    or np.isinf(std_loss)
                    or np.isnan(std_loss)
                ):  # Ensure mean/std are valid
                    processed_loss_list = [
                        l
                        for l in loss_list
                        if l is not None and l < mean_loss + filter_coeff * std_loss
                    ]

            # Filter out potential inf/nan before log if val_loss_log is True
            if val_loss_log:
                y_data_plot = [
                    np.log(l) for l in processed_loss_list if l is not None and l > 0
                ]
                x_data_plot = [
                    i
                    for i, l in enumerate(processed_loss_list)
                    if l is not None and l > 0
                ]
            else:
                y_data_plot = [l for l in processed_loss_list if l is not None]
                x_data_plot = [
                    i for i, l in enumerate(processed_loss_list) if l is not None
                ]

            if not x_data_plot:
                continue  # Skip if no valid data to plot

            label_text = (
                f"Fold{idx+1} val_log_loss" if val_loss_log else f"Fold{idx+1} val_loss"
            )
            sns.lineplot(
                x=x_data_plot,
                y=y_data_plot,
        ...
        plt.xlabel("Epochs")
        plt.ylabel("Log Loss" if val_loss_log else "Loss")
        plt.legend()
        plt.tight_layout()
        plt.grid(True, linestyle="--", alpha=0.7)
        ...
        for idx, f1_list in enumerate(self.total_fold_results["train_f1"]):
            if f1_list:
                sns.lineplot(
                    x=range(len(f1_list)),
                    y=f1_list,
                    label=f"Fold{idx+1} train_f1 (NN)",
                    palette=PALETTE,
                    alpha=ALPHA,
                )
        plt.title("Train F1 Score (NN)")
        plt.xlabel("Epochs")
        plt.ylabel("F1 Score")
        plt.legend()
        plt.grid(True, linestyle="--", alpha=0.7)
        ...
        for idx, f1_list in enumerate(self.total_fold_results["val_f1"]):  # NN val F1s
            if f1_list:
                sns.lineplot(
                    x=range(len(f1_list)),
                    y=f1_list,
                    label=f"Fold{idx+1} val_f1 (NN)",
                    palette=PALETTE,
                    alpha=ALPHA,
                )
        plt.title("Validation F1 Score (NN)")
        plt.xlabel("Epochs")
        plt.ylabel("F1 Score")
        plt.legend()
        plt.tight_layout()
        ...
        for idx, acc_list in enumerate(self.total_fold_results["train_accuracy"]):
            if acc_list:
                sns.lineplot(
                    ...
                    y=acc_list,
                    label=f"Fold{idx+1} train_accuracy (NN)",
                    palette=PALETTE,
                    alpha=ALPHA,
                )
        plt.title("Train Accuracy (NN)")
        plt.xlabel("Epochs")
        plt.ylabel("Accuracy")
        plt.legend()
        plt.grid(True, linestyle="--", alpha=0.7)
        ...
        for idx, acc_list in enumerate(
            ...
            if acc_list:
                sns.lineplot(
                    x=range(len(acc_list)),
                    y=acc_list,
                    label=f"Fold{idx+1} val_accuracy (NN)",
                    palette=PALETTE,
                    alpha=ALPHA,
                )
        plt.title("Validation Accuracy (NN)")
        plt.xlabel("Epochs")
        plt.ylabel("Accuracy")
        plt.legend()
        plt.tight_layout()
        ...
        if save:
            plt.savefig(
                os.path.join(base_path, "hybrid_train_val_accuracy_parallel.png")
            )
    ...
    def evaluate(self, X_test: DataFrame, y_test: Series, batch_size=32, plot=True):
        ...
        y_test_numpy = (
            y_test.to_numpy() if isinstance(y_test, pd.Series) else np.asarray(y_test)
        )
        ...
        if len(predictions) != len(y_test_numpy):
            print(
                f"Warning: Length mismatch between predictions ({len(predictions)}) and true labels ({len(y_test_numpy)}). Evaluation might be incorrect."
            )
            ...
            if (
                len(predictions) == 0 and len(y_test_numpy) > 0
            ):  # No predictions for non-empty y_test
                predictions = np.zeros_like(
                    y_test_numpy
                )  # Default to all zeros for calculation
        ...
        precision = precision_score(
            y_test_numpy, predictions, average="binary", zero_division=0
        )
        recall = recall_score(
            y_test_numpy, predictions, average="binary", zero_division=0
        )
        ...
        try:
            ...
            if (
                cm.shape != (2, 2) and len(np.unique(y_test_numpy)) <= 2
            ):  # Handle cases where only one class is predicted or present
                ...
                if len(unique_labels) == 1:  # Only one class present/predicted
                    ...
                    if cls in [0, 1]:  # Ensure it's one of the expected binary labels
                        ...
                        for i in range(len(predictions)):
                            if y_test_numpy[i] == 0 and predictions[i] == 0:
                                tn += 1
                            elif y_test_numpy[i] == 0 and predictions[i] == 1:
                                fp += 1
                            elif y_test_numpy[i] == 1 and predictions[i] == 0:
                                fn += 1
                            elif y_test_numpy[i] == 1 and predictions[i] == 1:
                                tp += 1
    ...
    def load_weights(self, model_path_prefix):
        ...
        if os.path.exists(ip_path):
            ...
            print(
                f"Warning: InteractionPipeline file not found: {ip_path}. self.ip remains None."
            )
            self.ip = None
