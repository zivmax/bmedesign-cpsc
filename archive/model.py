...
class HybridModel:
    ...
    def train(
        ...
        fold_iter = (
            tqdm(
                enumerate(skf.split(signal_df, target)),
                total=splits,
                desc="Folds",
                position=0,
            )
            if verbose
            else enumerate(skf.split(signal_df, target))
        )
        ...
        for fold, (train_idx, val_idx) in fold_iter:
            ...
            classifier_layer = NNClassifier(
                self.cnn_params["embedding_dim"], n_classes
            ).to(self.device)
            ...
            for epoch in epoch_iter:
                ...
                for batch_idx, (data, label) in batch_iter:
                    ...
                    if label.dim() > 1:
                        label = label.squeeze(
                            1
                        )  # Ensure label is 1D for CrossEntropyLoss
                ...
                if val_loss < best_val_loss:
                    ...
                    if patience_counter >= stopping_thershold:
                        if verbose:
                            tqdm.write(
                                f"Fold {fold+1} early stopping at epoch {epoch+1}"
                            )
                        break
            ...
            classifier = XGBClassifier(
                **self.classifier_params, **self.xgb_gpu_options, random_state=SEED
            )
            ...
            if verbose:
                tqdm.write(
                    f"Fold {fold+1}/{splits} completed in {fold_time:.2f}s | "
        ...
        best_model_idx = np.argmax(
            [model["val_xgb_f1"] for model in total_fold_results["best_models"]]
        )
        best_model = total_fold_results["best_models"][best_model_idx]

        (
            torch.save(
                best_model["cnn_state"],
                f"{datetime.datetime.now().strftime('%Y-%m-%d-%H-%M')}_Hybrid_CNN.pth",
            )
            if model_save_path
            else None
        )
    ...
    def train_process_plot(self, save=True, val_loss_log=False, filter_coeff=None):
        ...
        for idx, loss in enumerate(total_fold_results["train_loss"]):
            sns.lineplot(
                x=range(len(loss)),
                y=loss,
                label="Fold{} train_loss".format(idx + 1),
                palette=PALETTE,
                alpha=ALPHA,
            )
        ...
        if val_loss_log:
            for idx, loss in enumerate(total_fold_results["val_loss"]):
                if filter_coeff:
                    loss = [
                        l
                        for l in loss
                        if l < np.mean(loss) + filter_coeff * np.std(loss)
                    ]
                sns.lineplot(
                    x=range(len(loss)),
                    y=np.log(loss),
                    label="Fold{} val_log_loss".format(idx + 1),
                    palette=PALETTE,
                    alpha=ALPHA,
                )
        else:
            for idx, loss in enumerate(total_fold_results["val_loss"]):
                ...
                sns.lineplot(
                    x=range(len(loss)),
                    y=loss,
                    label="Fold{} val_loss".format(idx + 1),
                    palette=PALETTE,
                    alpha=ALPHA,
                )
        ...
        for idx, f1 in enumerate(total_fold_results["train_f1"]):
            sns.lineplot(
                x=range(len(f1)),
                y=f1,
                label="Fold{} train_f1".format(idx + 1),
                palette=PALETTE,
                alpha=ALPHA,
            )
        ...
        for idx, f1 in enumerate(total_fold_results["val_f1"]):
            sns.lineplot(
                x=range(len(f1)),
                y=f1,
                label="Fold{} val_f1".format(idx + 1),
                palette=PALETTE,
                alpha=ALPHA,
            )
    ...
    def evaluate(self, X_test, y_test, batch_size=32, plot=True):
        ...
        test_interaction = (
            self.ip.transform(X_test)
            if self.ip
            else ValueError("InteractionPipeline not fitted. Call train() first.")
        )
    ...
    def predict(self, X, batch_size=16):
        ...
        for i in range(0, num_samples, batch_size):
            ...
            X_tensor = (
                torch.tensor(batch_data.values, dtype=torch.float32)
                .unsqueeze(1)
                .to(self.device)
            )
